...
const DBTemplate = {//把这些数据库中的数据都拿出来封装成对象，用谁拿谁
    resumeObj: (obj) => {//简历表的对象
        return {
            resumeId: obj.resumeId, // 没用上
            custId: obj.custId, // 没用上
            resumeName: obj.resumeName,
            resumeCreateTime: obj.resumeCreateTime, // 没用上
            resumeUpdateTime: obj.resumeUpdateTime, // 没用上
            ...
            status: obj.status, // 没用上
            other1: obj.other1, // 没用上
            other2: obj.other2 // 没用上
        }
    },
    workExperienceObj: (obj) => {//工作经验
        return {
            workExperiId: obj.workExperiId, // 没用上
            resumeId: obj.resumeId, // 没用上
            workStartTime: obj.workStartTime,
            workEndTime: obj.workEndTime,
            workCompanyName: obj.workCompanyName,
            workPosition: obj.workPosition,
            workDepartment: obj.workDepartment,
            workCompanyScaler: obj.workCompanyScaler,
            workCompanyProperty: obj.workCompanyProperty,
            workDesc: obj.workDesc,
    ...
    projectExperienceObj: (obj) => {//项目经验
        return {
            projectExperiId: obj.projectExperiId, // 没用上
            resumeId: obj.resumeId, // 没用上
...
let EditTemplate = {
    basic: function (obj) {
        ...
        let content = `<div class="oh">
                            <div class="float_l">
                                <img src="${resume.resumeProfile}" alt="用户头像" id="face">
                            </div>
        ...
        if (resume.resumeLeavingWorking === "目前正在找工作")
            content += `<select name="resumeLeavingWorking" id="">
                            ...
                            <option value="waiting">观望有机会会考虑</option>
                            <option value="nochange">我目前不想换工作</option>
                        </select>`;
        else if (resume.resumeLeavingWorking === "观望有机会会考虑")
            content += `<select name="resumeLeavingWorking" id="">
                            <option value="">请选择</option>
                            <option value="目前正在找工作">目前正在找工作</option>
                            <option value="观望有机会会考虑" selected>观望有机会会考虑</option>
                            <option value="我目前不想换工作">我目前不想换工作</option>
                        </select>`;
        else if (resume.resumeLeavingWorking === "我目前不想换工作")
            content += `<select name="resumeLeavingWorking" id="">
                            <option value="">请选择</option>
                            <option value="目前正在找工作">目前正在找工作</option>
                            <option value="观望有机会会考虑">观望有机会会考虑</option>
                            <option value="我目前不想换工作" selected>我目前不想换工作</option>
                        </select>`;
        else
            content += `<select name="resumeLeavingWorking" id="">
                            <option value="">请选择</option>
        ...
        content += `</div></div>
                    <div class="oh">
                        <div class="float_l item_type1">
                            <label>居住地</label>
                            <input type="text" value="${resume.resumeLiveCity}" name="resumeLiveCity">
                        </div>
                        <div class="float_l item_type1">
                            <label>身份证号</label>
                            <input type="text" value="${resume.resumePersonid}" name="resumePersonid">
                        </div>
                    </div>
                    <div class="oh">
                        <div class="float_l item_type1">
    ...
    work: function (obj) {
        let {...temp} = EmptyObj.workExperienceObj;//同上一个一样
        let arr = Object.getOwnPropertyNames(temp);
        for (let i = 0; i < arr.length; i++) {
            temp[arr[i]] = obj[arr[i]] || ""
        }
        obj = temp;
        let workExObj = DBTemplate.workExperienceObj(obj);
        let output = document.createElement("form");
        output.id = "work_" + workExObj.workExperiId;
        output.className = "content";
        output.innerHTML = `<div class="oh">
                                    <div class="float_l item_type1">
                                        <label>开始时间</label>
                                        <input type="date" value="${dateFormat(workExObj.workStartTime, "-")}" name="workStartTime">
                                    </div>
                                    <div class="float_l item_type1">
                                        <label>结束时间</label>
                                        <input type="date" value="${dateFormat(workExObj.workEndTime, "-")}" name="workEndTime">
                                    </div>
                                </div>
                       <div class="oh">
                           <div class="float_l item_type1">
                               <label>公司</label>
                               <input type="text" value="${workExObj.workCompanyName}" name="workCompanyName">
                           </div>
                           <div class="float_l item_type1">
                               <label>职位</label>
                               <input type="text" value="${workExObj.workPosition}" name="workPosition">
                           </div>
                       </div>
                       <div class="oh">
                           <div class="float_l item_type1">
                               <label>部门名称</label>
                               <input type="text" value="${workExObj.workDepartment}" name="workDepartment">
                           </div>
                           <div class="float_l item_type1">
                               <label>公司规模</label>
                               <input type="text" value="${workExObj.workCompanyScaler}" name="workCompanyScaler">
                           </div>
                       </div>
                       <div class="oh">
                           <div class="float_l item_type1">
                               <label>公司性质</label>
                               <input type="text" value="${workExObj.workCompanyProperty}" name="workCompanyProperty">
                           </div>
                           <div class="float_l item_type1">
                               <label>经验类型</label>
                               <input type="text" value="${workExObj.workType}" name="workType">
                           </div>
                       </div>
                       <div class="oh">
                           <div class="float_l item_type2">
                               ...
                               <textarea name="workDesc" rows="3">${workExObj.workDesc}</textarea>
                           </div>
                       </div>`;
        return output;
    },
...
let PrevTemplate = {
    basic: function (obj) {
        ...
        output.innerHTML = `<div class="oh">
                        ...
                        <div class="oh">
                            ...
                            <div class="float_l item_type1">
                                <label>证件号</label>
                                <span>${resume.resumePersonid}</span>
                            </div>
                        </div>
                        <div class="oh">
                            <div class="float_l item_type1">
    ...
    work: function (obj) {
        let {...temp} = EmptyObj.workExperienceObj;
        let arr = Object.getOwnPropertyNames(temp);
        for (let i = 0; i < arr.length; i++) {
            temp[arr[i]] = obj[arr[i]] || ""
        }
        ...
        output.innerHTML = `<div class="oh">
                            <div class="time_item">
                                <span>${dateFormat(workExObj.workStartTime, "/")}-${dateFormat(workExObj.workEndTime, "/")}</span>
                                <strong>${workExObj.workCompanyName}</strong>
                                <strong>${workExObj.workPosition}</strong>
                            </div>
                        </div>
                        <div class="oh">
                            <div class="item_type2 at">
                                <span>${workExObj.workType}</span>
                                <span>${workExObj.workCompanyScaler}</span>
                                <span>${workExObj.workCompanyProperty}</span>
                                <span>${workExObj.workDepartment}</span>
                            </div>
                        </div>
                        <div class="oh">
                            <div class="item_type2">
                                <label>工作描述</label>
                                <span>${workExObj.workDesc}</span>
                            </div>
                        </div>`;
        return output;
    },
