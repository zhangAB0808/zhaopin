...
<div class="column">
    ...
    <table class="box1" id="tab_basic">
        <tr>
            <td class="hbox" align="middle">
                <img src="${requestScope.resumeAllVO.resume.resumeProfile}" width="85" height="104" class="head"
                     alt="头像">
            </td>
            <td>
                        ...
                        <td valign="top" colspan="3">
                            <p style="margin-bottom: 5px">
                                <c:if test="${requestScope.resumeAllVO.resume.resumeGender != null}">
                                    <img alt="" class="vam"
                                         src="${pageContext.request.contextPath}/images/resumePreview/basic.png"
                                         width="20"
                                         height="20"/>
                                ...
                                <c:if test="${requestScope.resumeAllVO.resume.resumeLiveCity != null}">
                                    <span>现居住 ${requestScope.resumeAllVO.resume.resumeLiveCity}</span>
                                </c:if>
                                <c:if test="${requestScope.resumeAllVO.resume.resumeLiveCity != null}">
                                    <span class="p5">|</span>
                                </c:if>
                                <c:if test="${requestScope.resumeAllVO.resume.resumePersonid != null}">
                                    <span>身份证 ${requestScope.resumeAllVO.resume.resumePersonid}</span>
                                </c:if>
                            </p>
                        </td>
                    </tr>
    ...
    <!--工作经验-->
    <c:if test="${requestScope.resumeAllVO.workExperienceList != null}">
        <table class="box" id="tab_work">
            <tr>
                <td class="plate1">工作经验</td>
            </tr>
            <c:forEach var="work_exp" items="${requestScope.resumeAllVO.workExperienceList}">
                <tr class="tba">
                    <td class="p15">
                        <div class="col2">
                        <span class="time">
                            <fmt:formatDate value="${work_exp.workStartTime}" pattern="yyyy/MM/dd"/>
                            -
                            <fmt:formatDate value="${work_exp.workEndTime}" pattern="yyyy/MM/dd"/>
                        </span>
                            <strong class="time">${work_exp.workPosition}</strong>
                            <span>${work_exp.workDepartment}</span>
                        </div>
                        <div class="col2">
                            <fmt:formatDate value="${work_exp.workStartTime}" pattern="MM" var="workStartMonth"/>
                            <fmt:formatDate value="${work_exp.workEndTime}" pattern="MM" var="workEndMonth"/>
                            <fmt:formatDate value="${work_exp.workStartTime}" pattern="yyyy" var="workStartYear"/>
                            <fmt:formatDate value="${work_exp.workEndTime}" pattern="yyyy" var="workEndYear"/>
                            <span>
                             ${work_exp.workCompanyName}
                             [<fmt:formatNumber value="${12*(workEndYear-workStartYear)+workEndMonth-workStartMonth}"/>
                             个月]
                        </span>
                            <span class="p5">|</span>
                            <span>${work_exp.workCompanyScaler}</span>
                            <span class="p5">|</span>
                            <span>${work_exp.workCompanyProperty}</span>
                            <span class="p5">|</span>
                            <span>${work_exp.workType}</span>
                        </div>
                        <div class="col2">
                            <span class="keys">工作描述：</span>
                            <span class="txt1">${work_exp.workDesc}</span>
                        </div>
                    </td>
                </tr>
            </c:forEach>
